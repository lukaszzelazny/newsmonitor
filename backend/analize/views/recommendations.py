...
def get_all_recommendations():
    ...
    query = text(f"""
    SELECT * FROM (
        SELECT DISTINCT ON (ba.ticker, ba.brokerage_house, ba.price_old, ba.price_new)
            ba.id,
            ba.created_at,
            ba.brokerage_house,
            ba.price_old,
        ...
        ORDER BY ba.ticker, ba.brokerage_house, ba.price_old, ba.price_new, ba.created_at DESC
    ) AS sub
    ...
    with engine.connect() as conn:
        ...
        tickers = {row[7] for row in result if row[7]}
        
        # Batch fetch prices
        current_prices = get_current_prices(list(tickers))
