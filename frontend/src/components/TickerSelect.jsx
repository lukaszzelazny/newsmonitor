            ...
            <div className="max-h-32 overflow-y-auto border bg-white rounded p-1 text-xs mb-2">
                {filteredTickers.map(ticker => (
                    <label key={ticker.value} className="flex items-center p-1 hover:bg-gray-100 rounded">
                        <input type="checkbox" checked={selectedTickers.includes(ticker.value)} onChange={() => toggleTicker(ticker.value)} className="h-3 w-3 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                        <span className="ml-2 text-gray-700">{ticker.label}</span>
                    </label>
                ))}
            </div>
            <button onClick={handleSave} disabled={isLoading || selectedTickers.length === 0} className={`w-full px-2 py-1 text-xs font-semibold text-white rounded-md transition-colors ${(isLoading || selectedTickers.length === 0) ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
